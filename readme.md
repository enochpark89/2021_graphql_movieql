...
# 9.0 Wrapping a REST API with GraphQL

- We are going to use YTS movie API and wrap it in the graphQL.

For example:
https://yts.mx/api/v2/list_movies.json

- Instead of creating the fake db, we are going to fetch data from other sources called yts API.
db.js

```js
// fetch movies and transform it into .json file. 
...
- Fix the resolver to respond the query in a way that it gets movies from the API
resolver.js
```js
...
- Define which data we will take or send from schema.
